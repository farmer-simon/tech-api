...
func Upload(context *gin.Context, savePath string) (r bool, finnalSavePath interface{}) {
	fileName := context.GetString(consts.ValidatorPrefix + "name")
	fileType := context.PostForm("type")
	membersId, _ := cur_userinfo.GetHomeCurrentUserId(context)
	...
	if sequence := variable.SnowFlake.GetId(); sequence > 0 {
		...
		if saveErr = context.SaveUploadedFile(file, newSavePath+saveFileName); saveErr == nil {
			...
			attrs, _ := model.CreateAttrsFactory("").Insert(path, fileName, fileType, 0, int(membersId))
