...
func CreateUsersTokenCacheFactory(userId int64, platform string) *userTokenCacheRedis {
	redCli := redis_factory.GetOneRedisClient()
	if redCli == nil {
		return nil
	}
	return &userTokenCacheRedis{redisClient: redCli, userTokenKey: "token_userid_" + platform + "_" + strconv.FormatInt(userId, 10)}
}
