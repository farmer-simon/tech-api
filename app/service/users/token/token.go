...
func (u *userToken) GenerateToken(platform string, userid int64, params string, expireAt int64) (tokens string, err error) {
...
func (u *userToken) RecordLoginToken(userToken string) bool {
	if customClaims, err := u.userJwt.ParseToken(userToken); err == nil {
		return model.CreateOauthFactory("").RecordOauthToken(customClaims.Platform, customClaims.UserId, userToken, customClaims.ExpiresAt)
	} else {
		return false
	}
}
...
func (u *userToken) TokenIsMeetRefreshCondition(token string) bool {
	...
	case consts.JwtTokenOK, consts.JwtTokenExpired:
		...
		if model.CreateOauthFactory("").OauthRefreshConditionCheck(customClaims.Platform, customClaims.UserId, token) {
			return true
		}
	}
	return false
...
func (u *userToken) RefreshToken(oldToken, clientIp string) (newToken string, res bool) {
	...
	if newToken, err = u.userJwt.RefreshToken(oldToken, variable.ConfigYml.GetInt64("Token.JwtTokenRefreshExpireAt")); err == nil {
		if customClaims, err := u.userJwt.ParseToken(newToken); err == nil {
			if model.CreateOauthFactory("").OauthRefreshToken(customClaims.UserId, customClaims.ExpiresAt, oldToken, newToken, customClaims.Platform) {
...
func (u *userToken) IsEffective(token string) bool {
	...
	if consts.JwtTokenOK == code {
		...
		if variable.ConfigYml.GetInt("Token.IsCacheToRedis") == 1 {
			tokenRedisFact := token_cache_redis.CreateUsersTokenCacheFactory(customClaims.UserId, customClaims.Platform)
		...
		if model.CreateOauthFactory("").OauthCheckTokenIsOk(customClaims.UserId, token, customClaims.Platform) {
			return true
		}
	}
	return false
