...
type RecordAccept struct {
	...
	Attach    string `json:"attach" form:"attach" binding:"required"`
}
...
func (validVal RecordPost) CheckParams(context *gin.Context) {
	...
	if extraAddBindDataContext == nil {
		...
		(&home.Services{}).PostRecord(extraAddBindDataContext)
	}
}

func (validVal RecordClose) CheckParams(context *gin.Context) {
	...
	if extraAddBindDataContext == nil {
		...
		// 验证完成，调用控制器,并将验证器成员(字段)递给控制器，保持上下文数据一致性
		(&home.Services{}).CloseRecord(extraAddBindDataContext)
	}
}

func (validVal RecordAccept) CheckParams(context *gin.Context) {
	...
	if extraAddBindDataContext == nil {
		...
		// 验证完成，调用控制器,并将验证器成员(字段)递给控制器，保持上下文数据一致性
		(&home.Services{}).AcceptRecord(extraAddBindDataContext)
	}
}
...
func (validVal RecordIndex) CheckParams(context *gin.Context) {
	...
	if extraAddBindDataContext == nil {
		...
		// 验证完成，调用控制器,并将验证器成员(字段)递给控制器，保持上下文数据一致性
		(&home.Services{}).RecordIndex(extraAddBindDataContext)
	}
}
func (validVal RecordList) CheckParams(context *gin.Context) {
	...
	if extraAddBindDataContext == nil {
		...
		// 验证完成，调用控制器,并将验证器成员(字段)递给控制器，保持上下文数据一致性
		(&home.Services{}).RecordList(extraAddBindDataContext)
	}
}
