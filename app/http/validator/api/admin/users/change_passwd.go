...
type ChangePasswd struct {
	...
	NewPasswd string `json:"new_passwd" form:"new_passwd" binding:"required,min=8"`
	RePasswd  string `json:"re_passwd" form:"re_passwd" binding:"required"`
}

// 验证器语法，参见 Register.go文件，有详细说明

func (c ChangePasswd) CheckParams(context *gin.Context) {
	...
	if c.NewPasswd != c.RePasswd {
		response.CustomError(context, "两次输入的密码不一致")
		return
	}
	if c.Passwd == c.NewPasswd {
		response.CustomError(context, "新密码和旧密码相同")
		return
	}
	...
	if extraAddBindDataContext == nil {
		...
		(&admin.Users{}).ChangePasswd(extraAddBindDataContext)
	}
