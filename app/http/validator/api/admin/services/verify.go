...
type Verify struct {
	// 表单参数验证结构体支持匿名结构体嵌套
	data_type.Id
	State        float64 `form:"state" json:"state" binding:"min=1,max=3"`
	RejectReason string  `form:"reject_reason" json:"reject_reason"`
}

// 验证器语法，参见 Register.go文件，有详细说明

func (validVal Verify) CheckParams(context *gin.Context) {
	...
	if validVal.State == 1 && validVal.RejectReason == "" {
		response.CustomError(context, "请输入拒绝理由")
		return
	} else if validVal.State != 1 {
		validVal.RejectReason = ""
	}
	...
	if extraAddBindDataContext == nil {
		...
		(&admin.Services{}).Verify(extraAddBindDataContext)
	}
