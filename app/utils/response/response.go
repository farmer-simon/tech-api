...
func CustomError(c *gin.Context, msg string) {
	ReturnJson(c, http.StatusBadRequest, consts.ValidatorParamsCheckFailCode, msg, "")
	c.Abort()
}
...
func ValidatorError(c *gin.Context, err error) {
	if errs, ok := err.(validator.ValidationErrors); ok {
		...
		for _, v := range wrongParam {
			wrongString = append(wrongString, v)
		}
		ReturnJson(c, http.StatusBadRequest, consts.ValidatorParamsCheckFailCode, consts.ValidatorParamsCheckFailMsg+"\n"+strings.Join(wrongString, ","), wrongParam)
	} else {
		errStr := err.Error()
